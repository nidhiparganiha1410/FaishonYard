            ...
            {posts.map((post, i) => (
                <a href={`/blog/${post.slug}`} key={post.id} className={cn("group cursor-pointer block", i === 1 ? "md:-translate-y-8" : "md:translate-y-4")}>
                    <div className="aspect-[3/4] overflow-hidden rounded-xl mb-6 bg-brand-gray">
                        <ImageWithFallback src={post.featured_image || ''} alt={post.title} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-1000 grayscale group-hover:grayscale-0" />
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {[posts[1], posts[2]].map(post => (
                    <a href={`/blog/${post.slug}`} key={post.id} className="relative aspect-[16/9] overflow-hidden rounded-xl group cursor-pointer block">
                        <ImageWithFallback src={post.featured_image || ''} alt={post.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-1000" />
